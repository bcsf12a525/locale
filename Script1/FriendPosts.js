...
function Notify(data, hub) {
    ...
    var self = this;
    data = data || {};
    self.desc = data.Description || "OYE";
}
...
function myModel() {
    ...
    self.newNotifications = ko.observableArray();
    ...
    self.addPost = function () {
        ...
        msg = msg.value;
        self.hub.server.addPost({ "Message": msg, "PostedBy":id }).fail(function (err) {
            self.error(err);
        });
    }
    self.Func = function () {
    ...
    self.loadNewNotifications = function () {
        self.notifications(self.newNotifications().concat(self.notifications()));
    ...
    self.hub.client.newNotification = function (notify) {
        alert("adf");

        self.newNotifications.splice(0, 0, new Notify(notify, self.hub));
    }
